...
/// Reinterprets a generic field slice as `$field` when the types match.
#[cfg(all(target_arch = "x86_64", feature = "ifma"))]
macro_rules! batch_downcast {
    ($f:ty, $field:ty, $lhs:expr, $rhs:expr, $mul:ident) => {
        if core::any::TypeId::of::<$f>() == core::any::TypeId::of::<$field>() {
            // SAFETY: `TypeId` equality guarantees `$f` is exactly `$field`,
            // so the slices may be reinterpreted at their own type.
            #[allow(unsafe_code)]
            unsafe {
                let lhs =
                    core::slice::from_raw_parts_mut($lhs.as_mut_ptr() as *mut $field, $lhs.len());
                let rhs = core::slice::from_raw_parts($rhs.as_ptr() as *const $field, $rhs.len());
                $mul(lhs, rhs);
            }
            return;
        }
...
pub(crate) fn batch_mul_generic<F: ff::Field>(lhs: &mut [F], rhs: &[F]) {
    #[cfg(all(target_arch = "x86_64", feature = "ifma"))]
    {
        batch_downcast!(F, Fp, lhs, rhs, fp_mul_slice);
        batch_downcast!(F, Fq, lhs, rhs, fq_mul_slice);
    }
...
pub(crate) fn batch_sqr_generic<F: ff::Field>(x: &mut [F]) {
    #[cfg(all(target_arch = "x86_64", feature = "ifma"))]
    {
        if core::any::TypeId::of::<F>() == core::any::TypeId::of::<Fp>() {
            // SAFETY: `TypeId` equality guarantees `F` is exactly `Fp`.
            #[allow(unsafe_code)]
            unsafe {
                fp_sqr_slice(core::slice::from_raw_parts_mut(
                    x.as_mut_ptr() as *mut Fp,
                    x.len(),
                ));
            }
            return;
        }
        if core::any::TypeId::of::<F>() == core::any::TypeId::of::<Fq>() {
            // SAFETY: `TypeId` equality guarantees `F` is exactly `Fq`.
            #[allow(unsafe_code)]
            unsafe {
                fq_sqr_slice(core::slice::from_raw_parts_mut(
                    x.as_mut_ptr() as *mut Fq,
                    x.len(),
                ));
            }
            return;
...
// Inline-assembly Montgomery multiplication for ADX-capable x86_64 CPUs,
